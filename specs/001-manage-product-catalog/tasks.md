...
- [X] T001 [P] Add ProductStatus and domain error types in src/ProductCatalog.Domain/Products/ProductStatus.cs and src/ProductCatalog.Domain/Products/ProductDomainException.cs (FR-003, FR-004, FR-012, FR-014; US1-AS3, US1-AS4, US4-AS1)
- [X] T002 [P] Add Create Product aggregate unit tests covering normalization, case-insensitive SKU value equality, required fields, positive two-decimal price, Active initial state, authenticated audit identity, UTC timestamps, and version 1 in tests/ProductCatalog.Domain.UnitTests/Products/ProductCreationTests.cs (FR-002, FR-003, FR-004, FR-010; US1-AS1, US1-AS3, US1-AS4)
- [X] T003 [P] Write Product update unit tests proving SKU cannot be supplied or changed, only name/description/price change, invalid changes are atomic, and real changes advance audit/version in tests/ProductCatalog.Domain.UnitTests/Products/ProductUpdateTests.cs (FR-003, FR-004, FR-009, FR-010, FR-011; US3-AS1, US3-AS2, US3-AS4)
- [X] T004 [P] Write Product lifecycle unit tests proving activation/deactivation transitions and same-state idempotency without audit/version changes in tests/ProductCatalog.Domain.UnitTests/Products/ProductLifecycleTests.cs (FR-010, FR-011, FR-012, FR-013; US4-AS1, US4-AS2, US4-AS3, US4-AS5)
- [X] T005 Implement Product aggregate creation, immutable SKU normalization, validated detail updates, lifecycle transitions, trusted audit fields, and numeric version in src/ProductCatalog.Domain/Products/Product.cs (FR-002, FR-003, FR-004, FR-009, FR-010, FR-011, FR-012, FR-013; US1-AS1, US1-AS3, US1-AS4, US3-AS1, US3-AS2, US3-AS4, US4-AS1, US4-AS2, US4-AS3, US4-AS5)
...
**Approved scope correction (2026-09-03)**: PR 2 is limited to Product persistence. Durable audit
events, stores/projections, API authorization, ProblemDetails, and API composition remain unchecked
and are deferred to later vertical-slice PRs. Currency is configuration-only.
...
- [X] T009 [P] Define IApplicationDbContext and create ApplicationDbContext plus Product configuration with approved constraints, UTC conversion, and concurrency mapping in src/ProductCatalog.Application/Abstractions/Persistence/IApplicationDbContext.cs, src/ProductCatalog.Infrastructure/Persistence/ApplicationDbContext.cs, and src/ProductCatalog.Infrastructure/Persistence/Configurations/ProductConfiguration.cs (FR-003, FR-004, FR-010, FR-011, FR-012, FR-013; US1-AS1, US1-AS3, US3-AS3, US4-AS1)
...
- [X] T014 Register ApplicationDbContext and IApplicationDbContext with configuration-based connection strings and explicit MySQL 8.0.46 provider settings in src/ProductCatalog.Infrastructure/DependencyInjection.cs and src/ProductCatalog.Infrastructure/Persistence/ProductCatalogDatabase.cs (FR-003, FR-010, FR-011; US1-AS1, US3-AS3)
- [X] T015 Create and review the initial MySQL Products schema migration in src/ProductCatalog.Infrastructure/Persistence/Migrations/ (FR-003, FR-004, FR-010, FR-011, FR-013; US1-AS3, US3-AS3, US4-AS4)
- [X] T016 [P] Build the reusable `mysql:8.0.46` Testcontainer fixture in tests/ProductCatalog.Api.IntegrationTests/Infrastructure/MySqlFixture.cs (FR-003, FR-004, FR-011; US1-AS3, US3-AS3)
- [X] T017 Add clean-migration, Product round-trip, case-insensitive unique SKU, UTC, and concurrency integration tests in tests/ProductCatalog.Api.IntegrationTests/Infrastructure/PersistenceTests.cs (FR-003, FR-004, FR-010, FR-011, FR-013; US1-AS3, US3-AS3, US4-AS4)
